...
- The once-a-day automatic sync compares two timestamps in the same timezone. The sync log records UTC and the comparison used the local date, so east of Greenwich there was a window each night in which a sync that had just finished counted as yesterday's - and every tool call started another one, which is the behaviour the throttle exists to prevent. West of Greenwich the same mismatch could suppress syncing for most of a day.
